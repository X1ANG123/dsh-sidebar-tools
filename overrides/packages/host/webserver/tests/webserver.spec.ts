...
describe('real Loader composition', () => {
  it('refuses to be framed on every route it answers', { timeout: 60_000 }, async () => {
    const loaded = await loadComposition()
    const server = loaded.webServer
    server.register({ kind: 'exact', path: '/probe', handler: (_req, res) => { res.writeHead(200); res.end('PROBE') } })
    // The named route and the unmatched one: the header is on the funnel, not
    // on a route, so an embedded page cannot reach this origin in a frame and
    // inherit the app's own APIs.
    for (const path of ['/probe', '/nothing-here']) {
      const response = await request(server.port, path)
      expect(response.headers.get('x-frame-options'), path).toBe('DENY')
      expect(response.headers.get('content-security-policy'), path).toBe("frame-ancestors 'none'")
    }
  })
