...
export class WebServer extends Service {
  ...
  async [Service.init](): Promise<void> {
    const handle = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
      // This interface is never a frame. The sidebar's browser pane embeds
      // third-party pages with their own origin, which is what makes them work
      // — and which also means a page could navigate its frame here and become
      // same-origin with the app, the one path from embedded content to this
      // process's own routes. Refusing to be framed at all closes that path in
      // the browser, for every route including the fallback.
