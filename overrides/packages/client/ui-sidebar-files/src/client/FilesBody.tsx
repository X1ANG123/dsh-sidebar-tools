 ...
 * expanded ones.
 *
 * The pane shell — body, header, icon control, blocked state — comes from
 * ui-primitives, so this pane and every other Sidebar pane stay one design.
 */
...
import {
  ...
  PaneBody, PaneHeader, PaneIconButton, PaneStatus, PaneStatusLine,
...
export function FilesBody({
  ...
  if (cwd === undefined) {
    return (
      <PaneStatus data-files-state="no-workspace">
        <PaneStatusLine>{t('noWorkspace')}</PaneStatusLine>
      </PaneStatus>
    )
  }
  ...
  return (
    <PaneBody data-files-state="tree" data-files-root={state.root}>
      <PaneHeader>
        <IconFolderOpen16 className={css.icon} />
        <span className={css.name}>{title}</span>
        <PaneIconButton label={t('reload')} data-files-reload className={css.pushEnd} onClick={reload}>
