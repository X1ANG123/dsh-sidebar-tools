...
function box(kind: string, order: number, icon?: SidebarRightGuideBox['icon'], opensNewTab = false): SidebarRightGuideBox {
  return {
    kind,
    order,
    opensNewTab,
...
describe('GuideBody', () => {
  ...
  it('picking a box whose type declares fresh tabs opens beside the one already there', () => {
    const { view, openTab } = mountGuide([box('terminal', 20, undefined, true)])
    const entry = view.container.querySelector('[data-sidebar-right-guide-entry="terminal"]')
    if (entry === null) throw new Error('expected the terminal box')
    fireEvent.click(entry)
    // `revealIfOpened: false` is the difference: a second shell is a second tab,
    // not a focus on the shell already running.
    expect(openTab).toHaveBeenCalledWith('terminal', { replaceTab: true, revealIfOpened: false })
    cleanup()
  })
