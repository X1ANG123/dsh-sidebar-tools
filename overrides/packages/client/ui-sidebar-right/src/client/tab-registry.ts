...
export interface SidebarRightGuideBox extends SidebarRightGuideEntry {
  ...
  /** The contributing type's `opensNewTab`: picking it opens a new tab, not the open one. */
...
export interface SidebarRightTabDefinition {
  ...
  /**
   * Whether opening this type again places a new tab instead of focusing the one
   * already showing it. Defaults to false: a page type is one page, and reopening
   * it focuses what is there. A live session or a browser tab is not — a second
   * open is a second thing, which is exactly what the guide's entry box asks for.
   */
  readonly opensNewTab?: boolean
}
...
export class SidebarRightTabRegistry {
  ...
  private refresh(): void {
    ...
    this.guideEntries = this.cached
      .flatMap(definition => (definition.guide ?? []).map(entry => ({ ...entry, kind: definition.kind, opensNewTab: definition.opensNewTab === true })))
