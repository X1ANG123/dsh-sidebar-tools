  ...
  const options = {
    ...
    fallback: (
      <ShippedGuide
        entries={entries}
        onPick={(entry) => {
          // A type that declares `opensNewTab` is opened beside the one already
          // there; every other type keeps the focus-an-open-page behavior.
          tab.actions.openTab(entry.kind, {
            replaceTab: true,
            ...(entry.opensNewTab ? { revealIfOpened: false } : {}),
          })
        }}
        t={t}
      />
